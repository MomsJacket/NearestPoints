...
std::vector<std::pair<size_t, Point3D>> FindNearestPointsToPolyline(const Polyline3D& poly, const Point3D& point){
    ...
    for (size_t i = 0; i < n - 1; ++i){
        ...
        Point3D proj = PointProjectionOnLineThroughSegment(point, SegI);
        if (IsProjectionInSegment(proj, SegI)){
            double dist = DistanceBetweenPoints(proj, point);
            distances.insert({proj, std::pair{i, dist}});
            if (dist < min_distance) min_distance = dist;
        }
        else{
            double dist1 = DistanceBetweenPoints(nodes[i], point);
            double dist2 = DistanceBetweenPoints(nodes[i + 1], point);
            if (dist1 < dist2) {
